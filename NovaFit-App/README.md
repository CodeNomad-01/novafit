...
4. En **Project Settings → API** copia:
   ...
   y pégalos en `NovaFit-App/.env.local`.

## 3. Desplegar en Vercel (5 min)

1. Sube el repo a GitHub.
2. En <https://vercel.com> → **Add New → Project** → conecta el repo.
3. **Root Directory**: `NovaFit-App` (el `package.json` raíz es solo un proxy).
4. En **Environment Variables** añade `NEXT_PUBLIC_SUPABASE_URL` y
   `NEXT_PUBLIC_SUPABASE_ANON_KEY`.
5. Deploy. Tendrás `https://tu-app.vercel.app` en ~60 segundos.
